...
Although having effective navigation techniques is important they are useless without some method of travel. Bowman et al. have created a list of aims for effective travel techniques.\cite{Bowman1997} These include, speed, accuracy, spatial awareness, ease of learning, ease of use, information gathering and presence. We suggest that user comfort should also be taken into account especially when designing a VE interface for 6-DOF controllers.
 ...
 Bowman et al. then performed a series of experiments where they found that tool directed navigation was faster than gaze directed navigation for navigating relative to objects and equal for navigating to an absolute point in space\cite{Bowman1997}. Additionally since gesture directed steering allows the user to freely observe their environment during travel it allows for superior spatial awareness and information gathering. We therefore suggest that we should prefer gesture directed steering techniques.
 
 Once a method for choosing a destination is implemented users are able to  within a VE in a number of ways which can be described in terms of metaphors. These include the flying metaphor and the raycasting metaphor\cite{Bowman1997}. The flying metaphor allows the user to move smoothly in the direction selected while the raycasting metaphor allows the user to select a point and then move there instantly. A variation on the flying metaphor is walking in which the user cannot rise above a perceived surface and must move along this surface in order to travel. There are advantages and disadvantages to these techniques. Raycasting or teleportation is fast but can lead to user disorientation\cite{Bowman1997}. Flying is not as fast but reduces the likelihood of disorientation, however due to the fact that simulator sickness is a concern within VEs\cite{SousaSantos2009} it is possible fast flight may lead to this negative effect. Walking is much slower and therefore less likely to lead to simulator sickness but is significantly slower than both other techniques.
...
The primary purpose of this research is to establish whether using a VE interface is advantageous over using a desktop interface for interactively modelling terrain. Specifically for terrain models to be used in VE applications. Because of scope reasons we will be testing using university students with 3D modelling experience. Results may therefore be different from a professional environment where users have considerably more experience with desktop systems. However, since our participants have university level experience with 3D modelling they can be considered similar to entry level 3D designers. Additionally, this reduces bias towards the desktop system as there is less of an user experience difference between the two systems.
...
\textbf{Is it faster to create a terrain model with a predetermined set of features in a VE or using a desktop?}\\
%\begin{description}
