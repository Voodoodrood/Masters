...
\begin{figure}[H]
	...
	\caption{\hl{A 3D widget which allows the manipulation of a point on a ridge line}}
\end{figure} 
...
\hl{ Bowman et al. then performed a series of experiments where they found that tool directed navigation was faster than gaze directed navigation for navigating relative to objects and equal for navigating to an absolute point in space}\cite{Bowman1997}\hl{. Additionally since gesture directed steering allows the user to freely observe their environment during travel it allows for superior spatial awareness and information gathering. We therefore suggest that we should prefer gesture directed steering techniques.
 
 Once a method for choosing a destination is implemented users are able to  within a VE in a number of ways which can be described in terms of metaphors. These include the flying metaphor and the raycasting metaphor}\cite{Bowman1997}\hl{. The flying metaphor allows the user to move smoothly in the direction selected while the raycasting metaphor allows the user to select a point and then move there instantly. A variation on the flying metaphor is walking in which the user cannot rise above a perceived surface and must move along this surface in order to travel. There are advantages and disadvantages to these techniques. Raycasting or teleportation is fast but can lead to user disorientation}\cite{Bowman1997}\hl{. Flying is not as fast but reduces the likelihood of disorientation, however due to the fact that simulator sickness is a concern within VEs}\cite{SousaSantos2009}\hl{ it is possible fast flight may lead to this negative effect. Walking is much slower and therefore less likely to lead to simulator sickness but is significantly slower than both other techniques.}
...
\textbf{\hl{Is it faster to create a terrain model with a predetermined set of features in a VE or using a desktop?}}\\
%\begin{description}
...
The initial design will be based on the findings of previous work in the area of designing within VEs. We will then utilize a user-centred design process to iteratively improve the interface.\hl{ For this we will use feedback from expert users, individuals with professional 3D modelling backgrounds, to guide our design.The expert users will use Nielsen's heuristics}\cite{nielsen1990heuristic}\hl{ to evaluate the interface after being briefed on the use of the heuristic tools.} This will lead to a VE system that is optimized for workflow speed, usability and user experience. We will aim to maximise user comfort while allowing for terrain modelling to be done quickly and accurately. \\
...
\hl{The experiment will be a within-groups design. Users will know what is being measured (speed, accuracy etc.). We will use counterbalancing to avoid issues such as the practice effect. Participants will use both the existing desktop system and the VE system. The tasks for each system will be exactly the same.}

Participants will be selected via convenience sampling. They are unlikely to have had significant VE experience due to the limited availability of VE devices in South Africa. However, this is should not affect results as the training session will provide an opportunity for all users to familiarize themselves with the VE system. \hl{Participants will be comprised  of computer science and architecture students with 3D modelling experience. It is not feasible to use professional designers for our experiment as it would not be possible to find enough individuals for objective results. However, since our participants have university level experience with 3D modelling they can be considered similar to entry level designers.}
...
During the second session participants will be asked to perform terrain modelling tasks, such as replicating an existing terrain as closely as possible. \hl{They will base their designs off concept art to simulate the typical work-flow in industry.} Users will be monitored for speed and accuracy using telemetry. Additionally, the experiments will be followed by a usability questionnaire as well as questions related to VE issues such as fatigue, motion sickness and disorientation.
